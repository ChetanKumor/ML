    ...
    if st.sidebar.button("🚀 Train Models"):
        with st.spinner("Training models... This may take a while! ⏳"):
            try:
                ...
                st.write("Results dictionary from train_models:")
                st.json(results)
                st.write(f"Best model name: {best_model_name}")
                st.write(f"Best model object (type): {type(best_model_obj) if best_model_obj else 'None'}")
                ...
                for name, metrics_data in results.items():
                    # Populate leaderboard data for all models that ran, even if not 'best'
                    leaderboard_entry = {
                        "Model": name,
                    }
                    # Always include the primary metric if available
                    if primary_metric_key in metrics_data:
                        leaderboard_entry[primary_metric_key] = metrics_data[primary_metric_key]
                    else:
                        leaderboard_entry[primary_metric_key] = np.nan # Use NaN if primary metric is missing (should not happen if model trained)

                    # Add other relevant details from metrics_data['Details']
                    for detail_key, detail_value in metrics_data.get('Details', {}).items(): # Use .get to safely access 'Details'
                ...
                # Always create and display the DataFrame. It will be empty if no models trained.
                leaderboard_df = pd.DataFrame(leaderboard_data)

                if not leaderboard_df.empty:
                    # Sort by the primary metric if the column exists
                    if primary_metric_key in leaderboard_df.columns:
                        # Ensure ascending is correct: True for Accuracy (higher better), False for R2 Score (higher better)
                        ascending_sort = (primary_metric_key != "R2 Score") # Accuracy: True, R2 Score: False (because sorting by positive value)
                        leaderboard_df = leaderboard_df.sort_values(by=primary_metric_key, ascending=ascending_sort).reset_index(drop=True)
                    st.dataframe(leaderboard_df)
                else:
                    st.info("No models were successfully trained to display metrics.")
